    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.packages.store') }}">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control form-control-admin @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name') }}" required>
                @error('name')
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control form-control-admin @error('description') is-invalid @enderror" id="description" name="description" rows="3">{{ old('description') }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control form-control-admin @error('price') is-invalid @enderror" id="price" name="price" value="{{ old('price') }}" min="0" required>
                    @error('price')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control form-control-admin @error('min_dp') is-invalid @enderror" id="min_dp" name="min_dp" value="{{ old('min_dp') }}" min="0" required>
                    @error('min_dp')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="d-flex gap-2 mt-4">
                <button type="submit" class="btn btn-admin btn-admin-primary">Simpan</button>
